      ...
      do 10 i=1,layer
       do 30 j=1,npass(i)
        do 50 k=1,ncord(i,j)        
         ...
         if((px.gt.p1x.and.px.lt.p2x).or.(px.lt.p1x.and.px.gt.p2x)) then
...
10    continue 
	...
	subroutine mergept(ptclose,i1,j1,k1,layer,npass,ncord,pcd)
     ...
     &  pcd(i1,j1,k1,2)
      ...
      write(1100,'(3I2,2f15.6)') i,j,k,pcd(i,j,k,1),pcd(i,j,k,2)       
...
! if reaching to the right edge
            if(j.eq.npass(i)) then        
             ...
             p3x=ptr(i+1,1) 
             p3y=ptr(i+1,2)
             call area_4pt(p1x,p1y,p2x,p2y,p3x,p3y,p4x,p4y,areaps(i,j))
             call pscoord(i,j,p1x,p1y,p2x,p2y,p3x,p3y,p4x,p4y,pcd,ncord)
             goto 30
            endif            
...
! g - groove
	...
	dimension ptl(100,2),ptm(100,2),ptr(100,2)
      ...
      do 10 i=2,layer       
        ...
        ptr(i,1)=ptr(1,1)+(i-1)*delt_right*ptr(layer+1,1)/ptr(layer+1,2)
...
!  bottom 3 points
 	...
 	ptm(1,2) = 0.0 - reinf/3.0
      ...
      ptr(1,1) = rd + gap/2.0
      ptr(1,2) = 0.0
      write(1100,*) "bottom 3 points "
      ibot=1
      write(1100,*) ibot, ptl(1,1),ptl(1,2)
            write(1100,*) ibot, ptm(1,1),ptm(1,2)
                  write(1100,*) ibot, ptr(1,1),ptr(1,2)      
 !  top 3 points     
      jtop = layer + 1
 	...
 	ptr(jtop,2) = th
      ...
      write(1100,*) "top 3 points "    
      write(1100,*) jtop, ptl(layer+1,1),ptl(layer+1,2)
            write(1100,*) jtop,ptm(layer+1,1),ptm(layer+1,2)
                  write(1100,*)jtop, ptr(layer+1,1),ptr(layer+1,2)
      return
	end
!*************************************************
...
!-------Area Calculation-----------------------------------------------------
      subroutine parea_cal(i,j,layer,npass,ncord,pcd,pcent,areaps)    
      implicit real*8(a-h,o-z)
...
50      continue          
        ...
        do 70 k=1,ncord(i,j)           
          ...
          area=area+area3p
          write(1100,*) i,j,k,area3p
70        continue       
          areaps(i,j)=area
        write(1100,'("C,A",2I2,3F15.6)') i,j,pcent(i,j,1),
     $ pcent(i,j,2),areaps(i,j)
