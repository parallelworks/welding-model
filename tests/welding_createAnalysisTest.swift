...
app (file analysis_file, file ferr, file fout) createAnalysisFiles (file feweldIn, file feweldBC, file feweldPreHeat, file fMeshInp, file[] utils){
	python2 "utils/Analysis_file_create.py" strcat("--model_inp_file=",filename(fMeshInp))  strcat("--log_dir=",dirname(fout)) strcat("--out_dir=",dirname(analysis_file)) stderr=filename(ferr) stdout=filename(fout);
}
...
//file[] analysis_files	        <FixedArrayMapper; files = "results/case_1/model_material.in, results/case_1/model_ini_temperature.in, results/case_1/model_node.in, results/case_1/model_bc.in, results/case_1/model_ele8.in, results/case_1/model_ele6.in, results/case_1/model_ele4.in, results/case_1/model_group.in">; 
// <filesys_mapper;location="results/case_1", pattern="model_*.in">;
file analFile                   <strcat(caseOutDirs[i], "/analFiles.tar")>;
...
// (analFile,createAnalFilesErr, createAnalFilesOut) = createAnalysisFiles(feweldIn, feweldBC, feweldPreHeat, meshInp_files[i], utils);

app (file analysis_files, file ferr, file fout) createAnalysisFiles2 (file feweldIn, file feweldBC, file feweldPreHeat, file fMeshInp, file[] utils){
...
(analFile,createAnalFilesErr, createAnalFilesOut) = createAnalysisFiles2(feweldIn, feweldBC, feweldPreHeat, meshInp_files[i], utils);
